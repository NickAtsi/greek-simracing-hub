...
const Admin = () => {
  ...
  const [confirmAction, setConfirmAction] = useState<{ title: string; description: string; action: () => void } | null>(
    null,
  );
  ...
  const fetchRecentActivity = async () => {
    ...
    const { data: ra } = await supabase
      ...
      .select("id, title, created_at, views, author_id")
  ...
  const handleAdminReply = async () => {
    ...
    await supabase
      .from("support_messages" as any)
      .insert({ ticket_id: selectedTicket.id, sender_id: user.id, content: adminReply.trim(), is_admin: true });
    await supabase
      .from("support_tickets" as any)
  ...
  const toggleArticlePublish = async (id: string, published: boolean) => {
    await supabase
      .from("articles" as any)
      .update({ published: !published } as any)
      .eq("id", id);
    fetchArticles();
  };
  ...
  const deletePodcast = async (id: string) => {
    await supabase
      ...
      .delete()
      .eq("id", id);
    toast({ title: "Επεισόδιο διαγράφηκε" });
    fetchPodcasts();
  };
  ...
  const handleAddCategory = async () => {
    if (catForm.type === "article") {
      await supabase
        .from("article_categories" as any)
        .insert({ name: catForm.name, slug: catForm.slug, color: catForm.color });
      fetchCategories();
    } else {
  ...
  const handleDeleteUser = async (userId: string, displayName: string) => {
    if (
      !confirm(
        `Σίγουρα θέλεις να διαγράψεις τον χρήστη "${displayName || userId}";\n\nΑυτή η ενέργεια δεν μπορεί να αναιρεθεί!`,
      )
    )
      return;
  ...
  return (
    <div className="min-h-screen bg-background">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="divide-y divide-border/50">
                    {recentUsers.map((u: any) => (
                      <div key={u.id} className="flex items-center gap-3 px-5 py-3">
                        ...
                        <div className="flex items-center gap-2">
                          {u.is_approved ? (
                            <span className="inline-flex items-center rounded-full bg-green-500/15 px-2 py-0.5 text-[10px] font-bold text-green-500">
                              Εγκρίθηκε
                            </span>
                          ) : (
                ...
                <div className="rounded-xl border border-border bg-card">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                    ...
                    <button onClick={() => setTab("articles")} className="text-xs text-primary hover:underline">
                      Δες όλα →
                    </button>
                  </div>
                  <div className="divide-y divide-border/50">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="rounded-xl border border-border bg-card">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                    ...
                    <button onClick={() => setTab("forum")} className="text-xs text-primary hover:underline">
                      Δες όλα →
                    </button>
                  </div>
                  <div className="divide-y divide-border/50">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Διαχείριση εγκρίσεων, admin ρόλων και στοιχείων προφίλ.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="rounded-xl border border-border bg-card p-4">
                  ...
                  <p className="text-2xl font-display font-black text-primary">
                    {users.filter((u: any) => u.is_approved).length}
                  </p>
                </div>
                <div className="rounded-xl border border-border bg-card p-4">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-secondary/30 border-b border-border">
                      <th className="text-left px-4 py-3 font-display text-xs text-muted-foreground uppercase">
                        Χρήστης
                      </th>
                      <th className="text-left px-4 py-3 font-display text-xs text-muted-foreground uppercase">
                        Κατάσταση
                      ...
                      <th className="text-right px-4 py-3 font-display text-xs text-muted-foreground uppercase">
                        Ενέργειες
                      </th>
                    </tr>
                  </thead>
                  ...
                  <div key={a.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {!a.published && (
                          <span className="text-xs text-muted-foreground bg-secondary px-1.5 rounded">Draft</span>
                        )}
                        {a.article_categories && (
                          <span className="text-xs text-muted-foreground">[{a.article_categories.name}]</span>
                        )}
                      </div>
                      <p className="font-medium text-foreground text-sm truncate">{a.title}</p>
                      <p className="text-xs text-muted-foreground">
                        {new Date(a.created_at).toLocaleDateString("el-GR")} · {a.views || 0} views
                      </p>
                    </div>
                    <div className="flex items-center gap-1">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => toggleArticlePin(a.id, a.pinned)}
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => toggleArticlePublish(a.id, a.published)}
                        className="h-7 px-2 text-xs"
                      >
                  ...
                  <div key={t.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {t.forum_categories && (
                          <span className="text-xs text-muted-foreground">[{t.forum_categories.name}]</span>
                        )}
                      </div>
                      <p className="font-medium text-foreground text-sm truncate">{t.title}</p>
                      <p className="text-xs text-muted-foreground">
                        {new Date(t.created_at).toLocaleDateString("el-GR")} · {t.views || 0} views
                      </p>
                    </div>
                    <div className="flex items-center gap-1">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => toggleThreadPin(t.id, t.pinned)}
                        className="h-7 px-2 text-xs"
                      >
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => toggleThreadLock(t.id, t.locked)}
                        className="h-7 px-2 text-xs"
                      >
            ...
            <div>
              ...
              <div className="space-y-3">
                {podcasts.map((p: any) => (
                  <div
                    key={p.id}
                    className="flex items-center gap-4 rounded-xl border border-border bg-card p-4 group hover:border-primary/30 transition-colors"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        {!p.published && (
                          <span className="text-[10px] bg-secondary text-muted-foreground px-1.5 py-0.5 rounded font-display">
                            DRAFT
                          </span>
                        )}
                        <p className="font-medium text-foreground text-sm truncate">
                          EP#{p.episode_number || "?"} — {p.title}
                        </p>
                      </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <Button
                  ...
                  className="gap-2 bg-gradient-greek text-white hover:brightness-110"
                >
                ...
                <div className="space-y-2 max-h-[600px] overflow-y-auto">
                  {["all", "open", "in_progress", "resolved", "closed"].map((f) => (
                    <span
                      ...
                      className={`inline-flex mr-1 cursor-pointer rounded-full px-3 py-1 text-xs font-medium transition-colors ${ticketFilter === f ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}
                    >
                      {f === "all"
                        ? "Όλα"
                        : f === "open"
                            ...
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground mt-0.5">
                                  {user_profile?.display_name || user_profile?.username || "Άγνωστος"}
                                </p>
                              </div>
                              <span
                                className={`text-xs font-medium flex-shrink-0 ${ticket.status === "open" ? "text-amber-400" : ticket.status === "in_progress" ? "text-blue-400" : ticket.status === "resolved" ? "text-green-400" : "text-muted-foreground"}`}
                              >
                                {ticket.status}
                              </span>
                            </div>
                            <p className="text-xs text-muted-foreground mt-1">
                              {new Date(ticket.created_at).toLocaleDateString("el-GR")} · {ticket.priority}
                            </p>
                          </button>
                        );
                      })}
                  ...
                  <div className="rounded-xl border border-border bg-card flex flex-col" style={{ maxHeight: "600px" }}>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3">
                      {ticketMessages.map((msg: any) => (
                        <div key={msg.id} className={`flex gap-2 ${msg.is_admin ? "flex-row-reverse" : ""}`}>
                          <div
                            className={`max-w-[75%] rounded-xl px-3 py-2 text-sm ${msg.is_admin ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground"}`}
                          >
                            {!msg.is_admin && (
                    ...
                    <div className="p-4 border-t border-border flex gap-2">
                      ...
                      <Button
                        ...
                        className="bg-gradient-greek text-white hover:brightness-110 px-3"
                      >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <Button
                    ...
                    className="gap-1"
                  >
                    <Package className="h-3.5 w-3.5" /> Παραγγελίες ({shopOrders.length})
                  </Button>
                  <Button
                    variant={shopTab === "products" ? "default" : "outline"}
                    size="sm"
                    onClick={() => setShopTab("products")}
                    className="gap-1"
                  >
                    <ShoppingCart className="h-3.5 w-3.5" /> Προϊόντα ({shopProducts.length})
                  </Button>
                  <Button
                    size="sm"
                    onClick={() => {
                  ...
                  >
                    <Plus className="h-3.5 w-3.5" /> Νέο Προϊόν
                  </Button>
                </div>
              </div>
          ...
          {tab === "settings" && (
            <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                  <h3 className="font-display text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                    <Globe className="h-4 w-4 text-primary" /> Γενικές Ρυθμίσεις
                  </h3>
                  <div className="space-y-3">
                    {[
                      ...
                      {
                        key: "site_tagline",
                        label: "Tagline / Περιγραφή",
                        placeholder: "Η #1 Ελληνική πλατφόρμα SimRacing",
                        icon: FileText,
                      },
                      {
                        key: "footer_text",
                        label: "Footer Text",
                        placeholder: "Made with ❤️ in Greece",
                        icon: FileText,
                      },
                      {
                        key: "contact_email",
                        label: "Email Επικοινωνίας",
                        placeholder: "info@greeksimracers.gr",
                        icon: Mail,
                      },
                      {
                        key: "support_hours",
                        label: "Ώρες Υποστήριξης",
                        placeholder: "Δευτ–Παρ: 10:00–22:00",
                        icon: Clock,
                      },
                    ].map((field) => (
                      <div key={field.key} className="rounded-xl border border-border bg-card p-4">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            size="sm"
                            onClick={() => saveSiteSetting(field.key, siteSettings[field.key] || "")}
                ...
                <div>
                  <h3 className="font-display text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                    <Link2 className="h-4 w-4 text-primary" /> Social Links & Discord
                  </h3>
                  <div className="space-y-3">
                    {[
                      {
                        ...
                        placeholder: "459797812251590677",
                        icon: MessageSquare,
                      },
                      {
                        key: "discord_invite",
                  ...
                  {/* Toggles */}
                  <h3 className="font-display text-sm font-bold text-foreground mt-6 mb-3 flex items-center gap-2">
                    <Settings className="h-4 w-4 text-primary" /> Λειτουργικές Ρυθμίσεις
                  </h3>
                  <div className="space-y-3">
                    {[
                      {
                        ...
                        desc: "Εμφανίζει μήνυμα συντήρησης στους χρήστες",
                      },
                      {
                        key: "registration_enabled",
                        label: "Εγγραφές Ενεργές",
                        desc: "Επιτρέπει νέες εγγραφές χρηστών",
                      },
                    ].map((toggle) => (
                      <div
                        key={toggle.key}
                      ...
                      >
                        ...
                        >
                          <span
                            className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-card transition-transform ${siteSettings[toggle.key] === "true" ? "translate-x-5" : "translate-x-0"}`}
                          />
                        </button>
                      </div>
                    ))}
              ...
              <div className="mt-6 rounded-xl border border-primary/20 bg-primary/5 p-4 max-w-2xl">
                ...
                <p className="text-xs text-muted-foreground">
                  Κάθε αλλαγή εφαρμόζεται αμέσως σε: Footer, Σελίδα Επικοινωνίας, Discord Widget, Social Links. Οι
                  ρυθμίσεις αποθηκεύονται στη βάση δεδομένων.
                </p>
              </div>
            </div>
          )}
      ...
      <Dialog
        ...
        onOpenChange={(v) => {
          setShowPodcastForm(v);
          if (!v) setEditingPodcast(null);
        }}
      >
        <DialogContent className="max-w-lg bg-card border-border">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setPodcastForm((p) => ({ ...p, episode_number: e.target.value }))}
                  className="bg-secondary/50"
                />
              </div>
              <div>
                ...
                <Input
                  placeholder="π.χ. 45:30"
                  value={podcastForm.duration}
                  onChange={(e) => setPodcastForm((p) => ({ ...p, duration: e.target.value }))}
                  className="bg-secondary/50"
                />
              </div>
            </div>
            <div>
              ...
              <Input
                placeholder="π.χ. Γιάννης Κ."
                value={podcastForm.host}
                onChange={(e) => setPodcastForm((p) => ({ ...p, host: e.target.value }))}
                className="bg-secondary/50"
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setPodcastForm((p) => ({ ...p, spotify_url: e.target.value }))}
                className="bg-secondary/50"
              />
            </div>
            <div>
              ...
              <Input
                placeholder="π.χ. Γενικά, F1, SimRacing..."
                value={podcastForm.category}
                onChange={(e) => setPodcastForm((p) => ({ ...p, category: e.target.value }))}
                className="bg-secondary/50"
              />
            </div>
            <div>
            ...
            <div className="flex items-center justify-between rounded-lg border border-border bg-secondary/20 px-4 py-3">
              ...
              >
                <span
                  className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-card transition-transform ${podcastForm.published ? "translate-x-5" : "translate-x-0"}`}
                />
              </button>
            </div>
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Button
                ...
                disabled={!podcastForm.title.trim()}
                className="bg-gradient-greek text-white hover:brightness-110 gap-2"
              >
                {editingPodcast ? (
                  <>
                    <Edit className="h-4 w-4" /> Ενημέρωση
                  </>
                ) : (
                  <>
        ...
        <DialogContent className="bg-card border-border">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleAddCategory} className="bg-gradient-greek text-white hover:brightness-110">
                Προσθήκη
              </Button>
            </div>
          </div>
      ...
      <Dialog open={!!editProfile} onOpenChange={() => setEditProfile(null)}>
        <DialogContent className="bg-card border-border">
          ...
          <div className="space-y-3">
            ...
            />
            <Input
              placeholder="Αγαπημένο Sim"
              value={profileForm.favorite_sim}
              onChange={(e) => setProfileForm((p) => ({ ...p, favorite_sim: e.target.value }))}
              className="bg-secondary/50"
            />
            <Input
              placeholder="Αγαπημένη Πίστα"
              value={profileForm.favorite_track}
              onChange={(e) => setProfileForm((p) => ({ ...p, favorite_track: e.target.value }))}
              className="bg-secondary/50"
            />
            <Input
              placeholder="Τύπος Setup"
              value={profileForm.setup_type}
              onChange={(e) => setProfileForm((p) => ({ ...p, setup_type: e.target.value }))}
              className="bg-secondary/50"
            />
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleSaveProfile} className="bg-gradient-greek text-white hover:brightness-110">
                Αποθήκευση
              </Button>
            </div>
          </div>
      ...
      >
        <DialogContent className="max-w-lg bg-card border-border">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between rounded-lg border border-border bg-secondary/20 px-4 py-3">
              <div>
                <p className="text-sm font-medium text-foreground">Ενεργό</p>
                <p className="text-xs text-muted-foreground">Εμφανίζεται στο Shop</p>
              </div>
              <button
                onClick={() => setProductForm((p) => ({ ...p, active: !p.active }))}
                className={`relative h-6 w-11 rounded-full transition-colors ${productForm.active ? "bg-primary" : "bg-secondary"}`}
              >
                <span
                  className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-card transition-transform ${productForm.active ? "translate-x-5" : "translate-x-0"}`}
                />
              </button>
            </div>
            <div className="flex justify-end gap-2">
