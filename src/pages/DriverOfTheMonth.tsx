...
import { Trophy, Vote, Star, Calendar, ChevronLeft, ChevronRight } from "lucide-react";
...
const DriverOfTheMonth = () => {
  ...
  const [nomForm, setNomForm] = useState({ driver_name: "", reason: "" });
  const [loading, setLoading] = useState(true);

  const fetchData = async () => {
    setLoading(true);
    const [{ data: noms }, { data: vts }] = await Promise.all([
  ...
  const handleNominate = async () => {
    ...
    if (!nomForm.driver_name.trim()) { toast({ title: "Συμπλήρωσε το όνομα", variant: "destructive" }); return; }
    const { error } = await supabase.from("driver_of_month_nominations" as any).insert({
      month_year: monthYear,
      driver_name: nomForm.driver_name.trim(),
      nominated_by: user.id,
      reason: nomForm.reason.trim() || null,
    } as any);
    if (error) {
      toast({ title: error.code === "23505" ? "Έχεις ήδη υποψηφίσει αυτόν τον οδηγό" : "Σφάλμα", variant: "destructive" });
    } else {
      toast({ title: "Υποψηφιότητα καταχωρήθηκε! 🏁" });
      setNomForm({ driver_name: "", reason: "" });
  ...
  const handleVote = async (nominationId: string) => {
    ...
    if (error) {
      toast({ title: "Σφάλμα ψήφου", variant: "destructive" });
    } else {
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="max-w-md mx-auto mb-10">
              <div className="rounded-xl border border-border/60 bg-card p-6 space-y-4">
                ...
                <Input
                  ...
                  value={nomForm.driver_name}
                  onChange={(e) => setNomForm({ ...nomForm, driver_name: e.target.value })}
                />
                <Textarea
                ...
                <div className="flex gap-2">
                  <Button onClick={handleNominate} className="flex-1">Υποβολή</Button>
