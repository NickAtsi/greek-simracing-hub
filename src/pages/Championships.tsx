...
const ChampionshipCard = ({ champ, index }: { champ: Championship; index: number }) => {
  const status = statusConfig[champ.status] || statusConfig.upcoming;
  const progress = champ.races_total > 0 ? (champ.races_completed / champ.races_total) * 100 : 0;
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5">
                ...
                Αγώνες {champ.races_completed}/{champ.races_total}
              </span>
              <span className="flex items-center gap-1.5">
              ...
              {champ.start_date && (
                <span className="flex items-center gap-1.5">
                  ...
                  {new Date(champ.start_date).toLocaleDateString("el-GR", { month: "short", year: "numeric" })}
                </span>
              )}
            </div>
      ...
      <section className="container mx-auto px-4 pb-24">
        ...
        >
          {[
            ...
            { icon: Flag, label: "Αγώνες", value: championships.reduce((a, c) => a + c.races_total, 0) },
            { icon: Users, label: "Οδηγοί", value: championships.reduce((a, c) => a + c.participants, 0) },
            { icon: MapPin, label: "Κατηγορίες", value: [...new Set(championships.map(c => c.category))].length },
          ].map((stat) => (
